...
public class Phone {
    ...
    // 고유 속성
    String model; // 모델명 // String은 class String
    String color; // 색상
    int price; // 가격 (출고가는 같다, 판매가는 변하지만)
    ...
    // 부품 속성
    // 문자메시지함
    String[] receiveMessage = new String[3];      // end field



    // 생성자 (constructor) : 객체의 초기화를 담당하며, 객체 생성시 반드시 호출
    // 하나도 선언하지 않을 시 자동으로 가본 생성자가 만들어진다.
    // 생성자는 class이름과 대소문자까지 같게 만들어야한다
    Phone() { // 매개변수가 없는 생성자 => 기본 생성자 // 태생적으로 void, void도 적지 않는다
    ...
    void sendMessage(Phone targetPhone, String msg) {
        // 상대방의 메시지함이 가득차있는가?
        if (targetPhone.receiveMessage.length > targetPhone.messageCount) {
            // 상대방 메시지함에 메시지 넣기
            targetPhone.receiveMessage[targetPhone.messageCount++] = msg + " by " + model;
            // 메시지 하나 받았으면 messageCount++
