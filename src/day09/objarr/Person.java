...
public class Person {
    ...
    Person(String name) {
        this.name = name;
//        this.money = 0; // 기본값으로 설정되므로 적을 필요 없음
//        this.myAccount = null; // 기본값으로 설정되므로 적을 필요 없음
    }

    // 계좌 개설 기능
    void makeAccount(Bank bank) {
        // 신규 계좌 생성
        this.myAccount = new Account(this); // 내 정보 통째로 넘기기
        // new Account(new Person("김영희"); (이건 문법적으로 되지만 계좌는 내가 만드는 것이므로 논리적으로는 안됨)
    ...
    void deposit(int depositMoney) {
        if (money >= depositMoney) {
            ...
            System.out.printf("입금실패! %d원이 더 필요합니다 \n", depositMoney - this.money);
        }
    }
    ...
    // 잔액 조회 기능
    void checkBalance() {
        System.out.printf("%s님의 현재 예금 잔액은 %d원입니다.", this.name, this.myAccount.balance);
    }
